...
const getBeasiswaById = async (req, res) => {
  try {
    const { id } = req.params;
    const [beasiswa] = await getPool.query(
      "SELECT * FROM beasiswa WHERE id = ?",
      [id],
    );
  ...
  try {
    ...
    if (!nama || !deskripsi || !deadline || !jenis_beasiswa) {
      return res
        .status(400)
        .json({
          message: "Nama, deskripsi, deadline, dan jenis beasiswa harus diisi",
        });
    }
...
const updateBeasiswa = async (req, res) => {
  try {
    ...
    const [existingBeasiswa] = await getPool.query(
      "SELECT id FROM beasiswa WHERE id = ?",
      [id],
    );
